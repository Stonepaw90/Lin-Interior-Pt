...
if not variable_dict["ex 11.7"]:
    ...
    st.markdown("## Coefficient matrix A (Example 11.7)")
    # Standard
    st.markdown("The Example 11.7 matrix is shown. The program will append the identity matrix, converting everything to canonical form.")
    st.write(
        "Warning: Submitting while a box is being edited will return an error. Before submitting, press enter or tab to confirm your edits.")
    grid_height = 335
    response = AgGrid(
    ...
    if m_s > 0 and n_s > 0:
        ...
        st.write("This is your matrix. It has shape (", m_s, "*", n_s, ") If this is incorrect, submit again.")
        st.latex("A = " + sympy.latex(sympy.Matrix(matrix_small)))
        st.header("Input your problem data (Example 11.7)")
        st.write("Enter vectors using spaces between entries, e.g.,\"1 4 3 2\".")
        col = st.beta_columns(2)
        # col_help = 0
        with col[0]:
            b = np.array([float(i) for i in
                          st.text_input(f"Right-hand side b (a {m_s}-vector)", value="16 12 10").split(" ")])
        n_full = n_s + m_s
        #If standard form, give small vectors. Else, give big vectors.
        with col[1]:
            c = np.array([float(i) for i in
                          st.text_input(f"Objective function coefficients c (a {n_s}-vector)", value="4 3").split(
                              " ")])
        st.header("Initial solution (Example 11.7)")
        col = st.beta_columns(2)
        with col[0]:
            x = np.array([float(i) for i in
                          st.text_input(f"x (a {n_s}-vector)", value="3.0 3.0").split(" ")])
        with col[1]:
            y = np.array([float(i) for i in
                          st.text_input(f"y (a {m_s}-vector)", value="2.0 2.0 1.0").split(" ")])
        st.header("Parameters (Example 11.7)")
        col = st.beta_columns(2)
        with col[0]:
            st.write(r"""$\alpha$: Step size parameter.""")
            alpha = st.number_input(r"""""", value=0.9, step=0.01, min_value=0.0, max_value=0.999,
